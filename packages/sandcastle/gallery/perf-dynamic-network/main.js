...
// Mutation cadence — chosen to stress buffer updates without saturating the
// interval callback itself. Numbers here are per-tick counts; tick period is
// controlled by the setInterval calls further down.
const COLOUR_CHURN_PER_TICK = Math.floor(ASSET_COUNT * 0.05); // ~5% billboards
const LINE_COLOUR_CHURN_PER_TICK = Math.floor(CONNECTION_COUNT * 0.05); // ~5% lines
const REWIRE_CHURN_PER_SECOND = Math.floor(CONNECTION_COUNT * 0.01); // ~1% lines
...
// Simple white disc — colour comes from billboard.color so it can be tinted
// per-instance without swapping images.
const assetBodyImage = svgDataUri(`
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" width="100" height="100">
  <circle cx="50" cy="50" r="44" fill="#fff" stroke="#222" stroke-width="4"/>
</svg>
`);

// Local HSL helper — the shared util's getRandomColor is flat-RGB and produces
// muted pastels. This benchmark deliberately uses vivid HSL cycles so per-
// instance recolouring is visible at a glance.
...
for (let index = 0; index < ASSET_COUNT; index += 1) {
  ...
  const bodyBillboard = billboards.add({
    id: { type: "asset", assetNumber },
    position,
    image: assetBodyImage,
    width: 24,
    height: 24,
    color: hslColor(Math.random()),
    horizontalOrigin: Cesium.HorizontalOrigin.CENTER,
    verticalOrigin: Cesium.VerticalOrigin.CENTER,
    disableDepthTestDistance: 100_000,
  });

  const nameLabel = showLabels
    ? labels.add({
        position,
        text: `Asset ${assetNumber}`,
        font: "12px sans-serif",
...
while (existingConnections.size < CONNECTION_COUNT) {
  const pair = pickTwoAssetIndexes();
  if (!pair) {
    continue;
  }
  const [startIndex, endIndex] = pair;
  ...
  // Give each polyline its own Material instance so its colour can be
  // updated independently — this is part of what we're benchmarking.
  const material = Cesium.Material.fromType("Color", {
    color: hslColor(random(), 0.7, 0.5, 0.35),
  });
  ...
  connectionRecords.push({ polyline, material, key, startIndex, endIndex });
}
...
viewer.scene.preRender.addEventListener(() => {
  ...
  for (let index = 0; index < ASSET_COUNT; index += 1) {
    ...
    record.bodyBillboard.position = record.currentPosition;
    if (record.nameLabel) {
      record.nameLabel.position = record.currentPosition;
    }
  }
...
// --- Colour churn: every 500 ms recolour a random subset ---------------
setInterval(() => {
  for (let i = 0; i < COLOUR_CHURN_PER_TICK; i += 1) {
    const idx = Math.floor(Math.random() * ASSET_COUNT);
    assetRecords[idx].bodyBillboard.color = hslColor(Math.random());
  }
  for (let i = 0; i < LINE_COLOUR_CHURN_PER_TICK; i += 1) {
    const idx = Math.floor(Math.random() * connectionRecords.length);
    connectionRecords[idx].material.uniforms.color = hslColor(
      Math.random(),
      0.7,
      0.5,
      0.35,
...
// --- Rewire churn: every 1 s move a subset of connections to new endpoints ---
setInterval(() => {
  for (let i = 0; i < REWIRE_CHURN_PER_SECOND; i += 1) {
    const recordIdx = Math.floor(Math.random() * connectionRecords.length);
    const record = connectionRecords[recordIdx];

    let pair = null;
    for (let attempt = 0; attempt < 8; attempt += 1) {
      const candidate = pickTwoAssetIndexes();
      if (!candidate) {
        continue;
      }
      const key = `${candidate[0]}:${candidate[1]}`;
    ...
    existingConnections.delete(record.key);
    existingConnections.add(pair.key);
    record.key = pair.key;
    record.startIndex = pair.indexes[0];
    record.endIndex = pair.indexes[1];
    record.polyline.positions = [
      currentPositions[record.startIndex],
      currentPositions[record.endIndex],
    ];
  }
}, 1000);
